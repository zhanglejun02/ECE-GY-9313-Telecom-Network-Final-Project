...
# Model path (Qwen3-30B-A3B MOE model)
# QWEN3_MODEL_PATH = "/maindata/data/shared/ai_story_workspace-dsw/common_models/Qwen/Qwen3-30B-A3B-Instruct-2507"
# QWEN3_MODEL_PATH = "/maindata/data/shared/ai_story_workspace-dsw/common_models/google/gemma-7b"
QWEN3_MODEL_PATH = "/aisocial-nlp/common_models/Qwen/Qwen3-30B-A3B-Instruct-2507"
...
def extract_model_name(model_path):
    ...
    # Extract the last part of the path (e.g., 'gemma-7b' from '/aisocial-nlp/common_models/google/gemma-7b')
...
def train_epoch(model, dataloader, optimizer, scheduler, device, accumulation_steps=1):
    ...
    # Force bfloat16, fallback to float32 if GPU doesn't support bfloat16 (e.g., P40, V100)
    ...
    for step, batch in enumerate(pbar):
        ...
        # Fix 1: Explicitly specify dtype as bfloat16 to avoid float16 overflow
        # Fix 2: Removed GradScaler (not needed for BF16)
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=desc):
            ...
            with torch.amp.autocast('cuda', dtype=dtype):
                outputs = model(**batch)
                logits = outputs.logits
            all_logits.append(logits.float().cpu().numpy()) # Convert back to float32 for numpy computation
...
def main():
    ...
    parser.add_argument("--use_flash_attn", default=True, action="store_true", help="Use Flash Attention 2 for faster training and lower memory usage")
    args = parser.parse_args()

    # Extract model name from model path
    model_name = extract_model_name(QWEN3_MODEL_PATH)
    
    # Create directories with model name included
    os.makedirs(f"./lora_models/{model_name}/{args.dataset}", exist_ok=True)
    ...
    # ================= Load Data =================
    # Load all CSV files once at the beginning (before training starts)
    ...
    # Extract labels once to avoid repeated CSV reading during evaluation
    y_train = np.array(train_df['target'])
    y_valid = np.array(valid_df['target'])
    y_test = np.array(test_df['target'])
    y_open = np.array([args.num_labels] * len(open_df))

    # ================= Load Model & LoRA =================
    print(f"Loading model from {QWEN3_MODEL_PATH}")
    tokenizer = AutoTokenizer.from_pretrained(QWEN3_MODEL_PATH, trust_remote_code=True)
    
    # Force model to load with BFloat16
    torch_dtype = torch.bfloat16
    
    # Check and configure Flash Attention
    attn_implementation = None
    if args.use_flash_attn:
        try:
            import flash_attn
            attn_implementation = "flash_attention_2"
            print("Flash Attention 2 enabled - faster training and lower memory usage")
        except ImportError:
            print("Warning: Flash Attention 2 requested but not installed. Falling back to default attention.")
    ...
    # LoRA configuration for Qwen3 MOE model
    # Note: For MOE models, the target_modules remain the same as standard Qwen architecture
    peft_config = LoraConfig(
        ...
        modules_to_save=["score"]  # Save the classification head (score layer)
    )
    ...
    # Ensure classification head uses bfloat16
    for name, param in model.named_parameters():
        if "score" in name:  # Fixed: use "score" instead of "classifier_head"
            param.data = param.data.to(torch.bfloat16)

    # ================= Prepare DataLoader =================
    collator = Qwen_collator(tokenizer, max_seq_len=args.max_len)
    ...
    for epoch in range(args.epochs):
        ...
        # Simple NaN check: if the entire epoch has NaN loss, stop training
        if pd.isna(train_loss):
            print("Error: Train loss is NaN. Stopping training.")
            break

        # Save checkpoint
        save_path = f"./lora_models/{model_name}/{args.dataset}/epoch_{epoch+1}"
        ...
        # Extract logits for all datasets (labels were already loaded at the beginning)
        ...
        with open(f"./results/{model_name}/{args.dataset}_lora_history.json", "w") as f:
